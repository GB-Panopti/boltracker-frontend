    ...
    <>
      <div className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-10 xl:grid-cols-5">
        ...
        <div className="col-span-1 md:col-span-1">
          <h3 className="text-gray-500 dark:text-gray-400">{t("billing.subscription_type")}</h3>
        ...
        {/* Subscribed Since */}
        <div className="col-span-1 md:col-span-1">
          <h3 className="text-gray-500 dark:text-gray-400">{t("billing.subscribed_since")}</h3>
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-50">
            {user.subscribed_since}
          </p>
        </div>

        {/* Billing Date */}
        <div className="col-span-1 md:col-span-1">
          <h3 className="text-gray-500 dark:text-gray-400">{t("billing.billing_date")}</h3>
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-50">
            {user.billing_date}
          </p>
        </div>
        ...
        {/* Billing Method */}
        <div className="col-span-1 md:col-span-1">
          <h3 className="text-gray-500 dark:text-gray-400">{t("billing.billing_method")}</h3>
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-50">
            {user.billing_method}
          </p>
        </div>
      ...
      <div className="">
        <Dialog>
          ...
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>{t("billing.confirm_cancel_title")}</DialogTitle>
              <DialogDescription className="mt-1 text-sm leading-6 grid-flow-row">
                <div>{t("billing.confirm_cancel_description")}</div>
                <div>
                    {t("billing.confirm_cancel_feedback")}
                    <Input type="text" placeholder={t("billing.confirm_cancel_feedback_placeholder")} />
                </div>
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="mt-6">
              ...
              <DialogClose asChild>
                ...
                >
                  {t("billing.confirm_cancel_button")}
                </Button>
              </DialogClose>
            </DialogFooter>
          </DialogContent>
